...
# File:   start_syslog-ng_valgrind.sh
# Date:   2026-05-29
#
# Helper to start syslog-ng daemon under Valgrind fpr mempry leak analysis
...
ssn_start_syslog() {
    ...
    echo " "
    echo "----------------------------------------"
    echo "--- START syslog-ng under valgrind"
    echo "----------------------------------------"
    echo " "
    ...
    VALGRIND_LOG="${TARGET_PATH}/valgrind-syslog-ng.log"
    echo "Valgrind log file: ${VALGRIND_LOG}"
    ...
    if pgrep -x "${SYSLOGNGMAIN}" >/dev/null || pgrep -f "valgrind.*syslog-ng" >/dev/null; then
