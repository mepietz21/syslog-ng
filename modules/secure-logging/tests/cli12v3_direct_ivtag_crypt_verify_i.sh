...
if [[ ${cnt_error} -eq 0 ]]; then
    ...
    while [[ ${i} -lt ${MAX_LOOP} ]]; do
        ...
        # Replace iv-tag-Base64 String by another valid Base64 Stirng
        TAMPERED_IV_TAG_BASE64=$(./tamper_b64.sh "${IV_TAG_BASE64}")
        echo "TAMPERED_IV_TAG_BASE64: ${TAMPERED_IV_TAG_BASE64}"

        NEW_TAMPERED="${LINE_COL}${TAMPERED_IV_TAG_BASE64}${MSG_DIRECT}"
        echo "NEW_TAMPERED: ${NEW_TAMPERED}"

        # Write back String
        # not working: sed -i "1s/.*/${NEW_TAMPERED}/" "${TEST}/plainlog_err_${i}.slog"
